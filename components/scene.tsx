  ...
  const { particles, originalPositionTexture, particlePositions } = useMemo(() => {
    ...
    for (let i = 0; i < size * size; i++) {
      ...
      if (positionAttribute && positionAttribute.count > 0) {
        const pIndex = i % positionAttribute.count
        particles[i4 + 0] = positionAttribute.getX(pIndex)
        particles[i4 + 1] = positionAttribute.getY(pIndex)
        particles[i4 + 2] = positionAttribute.getZ(pIndex)
      } else {
        particles[i4 + 0] = 0
        particles[i4 + 1] = 0
        particles[i4 + 2] = 0
